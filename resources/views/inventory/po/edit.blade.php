...
@section('title', 'Edit PO ' . $po->invoice_number)
...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-[20px] font-bold text-gray-800">Edit Purchase Order</h2>
            <p class="text-[13px] text-gray-400 mt-0.5">Ubah informasi atau item obat untuk faktur pembelian {{ $po->invoice_number }}.</p>
        </div>
    ...
    {{-- ===== FORM PO ===== --}}
    <form method="POST" action="{{ route('purchase-order.update', $po->id) }}" id="poForm">
        @csrf
        @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="supplier_id" required class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-[13px] text-gray-700 focus:outline-none focus:border-teal-500 transition-colors">
                                ...
                                @foreach($suppliers as $sup)
                                    <option value="{{ $sup->id }}" {{ old('supplier_id', $po->supplier_id) == $sup->id ? 'selected' : '' }}>{{ $sup->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-[12px] font-semibold text-gray-500 mb-1">Tanggal Pembelian <span class="text-red-500">*</span></label>
                            <input type="date" name="order_date" required value="{{ old('order_date', $po->order_date->format('Y-m-d')) }}" 
                            ...
                            <tbody id="itemsContainer" class="divide-y divide-gray-50">
                                @foreach($po->details as $index => $detail)
                                    <tr class="item-row align-middle">
                                        <td class="py-3 pr-2">
                                            ...
                                            <select name="items[{{ $index }}][medicine_id]" required onchange="calculateRowSubtotal(this)"
                                                ...
                                                @foreach($medicines as $med)
                                                    <option value="{{ $med->id }}" data-price="{{ $med->purchase_price }}" {{ $detail->medicine_id == $med->id ? 'selected' : '' }}>{{ $med->name }} ({{ $med->code }})</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td class="py-3 pr-2">
                                            <input type="text" name="items[{{ $index }}][batch_number]" value="{{ $detail->batch_number }}" placeholder="BATCH-..." required
                                                   class="w-full bg-gray-50 border border-gray-200 rounded-lg px-2 py-1.5 text-[12px] text-gray-700 focus:outline-none focus:border-teal-500 transition-colors">
                                        </td>
                                        <td class="py-3 pr-2">
                                            <input type="number" name="items[{{ $index }}][quantity]" value="{{ $detail->quantity }}" placeholder="0" min="1" required oninput="calculateRowSubtotal(this)"
                                        ...
                                        <td class="py-3 pr-2">
                                            <input type="number" name="items[{{ $index }}][purchase_price]" value="{{ round($detail->purchase_price) }}" placeholder="0" min="0" required oninput="calculateRowSubtotal(this)"
                                                   class="w-full bg-gray-50 border border-gray-200 rounded-lg px-2 py-1.5 text-[12px] text-right text-gray-700 focus:outline-none focus:border-teal-500 transition-colors">
                                        </td>
                                        <td class="py-3 pr-2 pl-3">
                                            <input type="date" name="items[{{ $index }}][expiry_date]" value="{{ $detail->expiry_date ? $detail->expiry_date->format('Y-m-d') : '' }}" required
                                                   class="w-full bg-gray-50 border border-gray-200 rounded-lg px-2 py-1.5 text-[12px] text-gray-700 focus:outline-none focus:border-teal-500 transition-colors">
                                        </td>
                                        <td class="py-3 text-right font-bold text-gray-700 text-[13px] subtotal-display" data-value="{{ $detail->subtotal }}">
                ...
                <div class="stat-card space-y-4 sticky top-5">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between text-[14px] border-t border-gray-100 pt-3">
                            ...
                            <span class="font-extrabold text-teal-700 text-[18px]" id="summaryGrandTotal">Rp {{ number_format($po->total_amount, 0, ',', '.') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="pt-2">
                        ...
                        <textarea name="notes" placeholder="Catatan pembelian..." rows="3" 
                                  class="w-full bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-[13px] text-gray-700 focus:outline-none focus:border-teal-500 transition-colors resize-none">{{ old('notes', $po->notes) }}</textarea>
                    </div>
                    ...
                    <button type="submit" class="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold text-[14px] py-3 rounded-lg transition-colors flex items-center justify-center gap-2 mt-2 shadow-md shadow-teal-600/10">
                        <i class="fa-solid fa-floppy-disk"></i> Simpan Perubahan PO
                    </button>
                </div>
            </div>
        </div>
